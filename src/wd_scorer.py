...
class CamieTaggerScorer:
    ...
    def get_tag_deviation_excluded_paths(
        ...
        """Exclude images where a tag is prominently present but absent in references.

        Only ``general`` category tags are checked (accessories, hair, etc.).
        ...
        """
        # Build index mask for general-category tags only
        general_mask = np.zeros(len(self.tag_names), dtype=bool)
        for tag, idx in self.tag_to_index.items():
            if self.tag_to_category.get(tag) == "general":
                general_mask[idx] = True
        ...
        for _emotion, items in emotion_groups.items():
            for path, _ in items:
                ...
                # Conditions: general tag, high prob, high positive deviation
                hits = np.where(
                    general_mask
